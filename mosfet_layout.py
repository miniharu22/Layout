...
## Drain Layer - 정확히 중앙에 정렬
l5 = layout.layer(5,0)
mosfet.shapes(l5).insert(pya.Box(2050, 7055, 2250, 7245))  # 중심 (2150, 7150)
mosfet.shapes(l5).insert(pya.Box(2650, 7055, 2850, 7245))  # 중심 (2750, 7150)
mosfet.shapes(l5).insert(pya.Box(3250, 7055, 3450, 7245))  # 중심 (3350, 7150)

## Source Layer - center aligned to upper contact
l6 = layout.layer(6,0)
mosfet.shapes(l6).insert(pya.Box(2050, 7822, 2250, 8037))
mosfet.shapes(l6).insert(pya.Box(2650, 7822, 2850, 8037))
mosfet.shapes(l6).insert(pya.Box(3250, 7822, 3450, 8037))

## Insert Text
gen = pya.TextGenerator()

text = gen.default_generator().text("NMOS", 0.002)
